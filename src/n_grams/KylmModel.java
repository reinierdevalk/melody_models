...
public class KylmModel implements ISequenceModelS, Serializable {
	...
	private int sliceIndex; // 1 = pitch; 2 = duration; 3 = relative pitch; 4 = ioi
	
	// NB: values must be between 1 and 9 because of conversion to String in ui.UI
	public static final int PITCH = 1; // rdv
	public static final int DUR = 2; // rdv
	public static final int REL_PITCH = 3; // rdv
	public static final int IOI = 4; // rdv
	...
	public double modelProbability(List<List<String>> list, int voiceNum) {		
		...
		if(lsType != Type.SHORT) {
    		...
    		probLT = Math.exp(entropy[entropy.length - 2]); // -2 because -1 is entropy of terminal symbol 
...
//			else {
				...
				try {
					tmpSTM.trainModel(voiceContexts); // train the SMT on all voices
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
	    		float[] entropy = tmpSTM.getWordEntropies(relPitchArray);
	    		probST = Math.exp(entropy[entropy.length -2]); // -2 because -1 is entropy of terminal symbol
//			}
		}
		if (lsType == Type.LONG ) 
			return probLT;
